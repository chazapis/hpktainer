...
const (
	// CacheHomeEnvVar is the environment variable used by HPK
	// for the cache directory. When no value is set a default is used.
	CacheHomeEnvVar = "HPK_CACHE_HOME"

	// ConfigHomeEnvVar is the environment variable used by HPK
	// for the config directory. When no value is set a default is used.
	ConfigHomeEnvVar = "HPK_CONFIG_HOME"

	// DataHomeEnvVar is the environment variable used by HPK
	// for the data directory. When no value is set a default is used.
	DataHomeEnvVar = "HPK_DATA_HOME"
)

// lazypath is a lazy-loaded path buffer for the XDG base directory specification.
type lazypath string

func (l lazypath) path(hpkEnvVar, xdgEnvVar string, defaultFn func() string, elem ...string) string {
	// There is an order to check for a path.
	// 1. See if a HPK specific environment variable has been set.
	// 2. Check if an XDG environment variable is set
	// 3. Fall back to a default
	base := os.Getenv(hpkEnvVar)
	if base != "" {
